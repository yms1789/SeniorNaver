...
function MemeDictionaryPracticeRandomSolvingList({
  useYear,
  problemList,
  setRandomCurrentStep,
}: IListProps) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    const result = await postResult(
      choiceResult.problemId,
      choiceResult.title,
      choiceResult.answer,
      choiceResult.choice,
    ); // 문제 결과를 저장합니다.
    if (result) {
      if (problemIndex === problemList.problemList.length - 1) {
        // 마지막 문제인지 확인
  ...
  return (
    <MemeDictionaryPracticeWraaper>
      {isLastProblem ? (
        <>
          <MemeDictionaryPracticeProblemExplanation>
            모든 문제를 풀이하셨습니다.
          </MemeDictionaryPracticeProblemExplanation>
        ...
        <>
          ...
          <MemeDictionaryPracticeFileFormArea>
            <MemeDictionaryPracticeFileFormImage
              src={currentProblem.image ? currentProblem.image : posefileformbutton}
            />
          </MemeDictionaryPracticeFileFormArea>
          <MemeDictionaryPracticeProblemExplanation>
            {currentProblem.problemExplanation}
          </MemeDictionaryPracticeProblemExplanation>
          <MemeDictionaryPracticeOptionsWrapper>
            <MemeDictionaryPracticeOptionsCircle
              isSelected={choiceResult.choice === 1}
              onClick={() => handleChoice(1)}
            >
              1
            </MemeDictionaryPracticeOptionsCircle>
            <MemeDictionaryPracticeOptionsInput isSelected={choiceResult.choice === 1}>
              {currentProblem.choices[0]?.content || ""}
            </MemeDictionaryPracticeOptionsInput>
          </MemeDictionaryPracticeOptionsWrapper>
          <MemeDictionaryPracticeOptionsWrapper>
            <MemeDictionaryPracticeOptionsCircle
              isSelected={choiceResult.choice === 2}
              onClick={() => handleChoice(2)}
            >
              2
            </MemeDictionaryPracticeOptionsCircle>
            <MemeDictionaryPracticeOptionsInput isSelected={choiceResult.choice === 2}>
              {currentProblem.choices[1]?.content || ""}
            </MemeDictionaryPracticeOptionsInput>
          </MemeDictionaryPracticeOptionsWrapper>
          <MemeDictionaryPracticeOptionsWrapper>
            <MemeDictionaryPracticeOptionsCircle
              isSelected={choiceResult.choice === 3}
              onClick={() => handleChoice(3)}
            >
              3
            </MemeDictionaryPracticeOptionsCircle>
            <MemeDictionaryPracticeOptionsInput isSelected={choiceResult.choice === 3}>
              {currentProblem.choices[2]?.content || ""}
            </MemeDictionaryPracticeOptionsInput>
          </MemeDictionaryPracticeOptionsWrapper>
          {completed ? (
