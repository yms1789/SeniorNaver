...
function MemeDictionaryPracticePoseProcessBox({ useYear }: { useYear: number }) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      word &&
      tags[0] &&
      choices.every(choice => choice.content)
    ) {
      setCompleted(true);
    } else {
      setCompleted(false);
  ...
  switch (currentStep) {
        ...
        <MemeDictionaryPracticeWraaper>
          {valid ? (
            ...
            <>
              ...
              <MemeDictionaryPracticeValidWordInput
                placeholder="단어 입력"
                name="word"
                value={newProblem.word}
    ...
    case 1:
      return (
        <MemeDictionaryPracticeWraaper>
          <MemeDictionaryPracticeProblemInput
            placeholder="문제 이름을 지어주세요"
            name="title"
          ...
          <MemeDictionaryPracticeForm onSubmit={handleSubmit}>
            <MemeDictionaryPracticeFileFormArea>
              ...
              <ProblemEditorLabel htmlFor="fileinput">
                <MemeDictionaryPracticeFileFormImage
                  src={problemFile ? problemFile.url : posefileformbutton}
                />
              </ProblemEditorLabel>
            </MemeDictionaryPracticeFileFormArea>

            <MemeDictionaryPracticeProblemExplanatioInput
              placeholder="문제 설명을 입력해주세요"
              name="problemExplanation"
              value={newProblem.problemExplanation}
              onChange={handleChange}
            />

            <MemeDictionaryPracticeOptionsWrapper>
              <MemeDictionaryPracticeOptionsCircle
                isSelected={newProblem.answer === 1}
                onClick={() => setProblem({ ...newProblem, answer: 1 })}
              >
                1
              </MemeDictionaryPracticeOptionsCircle>
              <MemeDictionaryPracticeOptionsInput
                isSelected={newProblem.answer === 1}
                placeholder="첫 번째 선지를 입력해주세요"
                onChange={handleChoiceChange(0)}
                value={newProblem.choices[0]?.content || ""}
              />
            </MemeDictionaryPracticeOptionsWrapper>

            <MemeDictionaryPracticeOptionsWrapper>
              <MemeDictionaryPracticeOptionsCircle
                isSelected={newProblem.answer === 2}
                onClick={() => setProblem({ ...newProblem, answer: 2 })}
              >
                2
              </MemeDictionaryPracticeOptionsCircle>
              <MemeDictionaryPracticeOptionsInput
                isSelected={newProblem.answer === 2}
                placeholder="두 번째 선지를 입력해주세요"
                onChange={handleChoiceChange(1)}
                value={newProblem.choices[1]?.content || ""}
              />
            </MemeDictionaryPracticeOptionsWrapper>

            <MemeDictionaryPracticeOptionsWrapper>
              <MemeDictionaryPracticeOptionsCircle
                isSelected={newProblem.answer === 3}
                onClick={() => setProblem({ ...newProblem, answer: 3 })}
              >
                3
              </MemeDictionaryPracticeOptionsCircle>
              <MemeDictionaryPracticeOptionsInput
                isSelected={newProblem.answer === 3}
                placeholder="세 번째 선지를 입력해주세요"
                onChange={handleChoiceChange(2)}
                value={newProblem.choices[2]?.content || ""}
              />
            </MemeDictionaryPracticeOptionsWrapper>
            ...
            <MemeDictionaryPracticeReviewInput
              type="text"
              placeholder="문제 풀이를 입력해주세요"
              name="review"
              value={newProblem.review}
              onChange={handleChange}
            />
    ...
    case 2:
      return (
        <MemeDictionaryPracticeWraaper>
          <MemeDictionaryPracticeCompletedProblem>
            "{newProblem.title}"
          </MemeDictionaryPracticeCompletedProblem>
          <MemeDictionaryPracticeHeader>출제 완료!</MemeDictionaryPracticeHeader>
          <MemeDictionaryPracticeText>
            출제한 문제는 나의 단어장에서 확인 가능합니다.
          </MemeDictionaryPracticeText>
        </MemeDictionaryPracticeWraaper>
      );
    default:
